...
class multi_ppo_barn:
    def __init__(self, ac_policy: ActorCritic, pi_lr=3e-4, vf_lr=1e-3, train_epoch=50, steps_per_epoch = 600, max_ep_len=300, gamma=0.99, lam=0.97, clip_ratio=0.2, train_pi_iters=100, train_v_iters=100, target_kl=0.01, render=False, render_freq=20, con_train=False, seed=7, save_freq=50, save_figure=False, save_path='test/', save_name='test', load_fname=None, device = 'mps', reset_mode=1, save_result=False, counter=0, test_env=None, lr_decay_epoch=1000, max_update_num=10, figure_save_path=None, **kwargs):
    ...
    def training_loop(self):
        ...
        obs_list, ep_ret_list, ep_len_list = self.env.reset(), [0] * self.robot_num, [0] * self.robot_num
        ...
        for epoch in tqdm(range(self.epoch + 1), desc='epoch'):
            ...
            for t in tqdm(range(self.steps_per_epoch), desc='time step'):
